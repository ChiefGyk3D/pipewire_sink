...
install_scripts() {
    ...
    if [ -f "$SCRIPT_DIR/examples/pipewire-ensure-defaults.sh" ]; then
        cp "$SCRIPT_DIR/examples/pipewire-ensure-defaults.sh" "$INSTALL_DIR/pipewire-ensure-defaults"
        chmod +x "$INSTALL_DIR/pipewire-ensure-defaults"
        success "Installed pipewire-ensure-defaults"
    fi
}

# Turn a concrete sink name into a stream.rules regex that survives
# replug suffix changes (USB "-00" enumeration) and profile switches.
sink_to_rule_regex() {
    local base="${1%.*}"                     # drop profile suffix (.analog-stereo, .hdmi-stereo, ...)
    base="$(echo "$base" | sed -E 's/-[0-9]+$//')"  # drop USB enumeration suffix
    echo "~${base}.*"
}

install_native_combined_sink_config() {
    info "Installing native combined sink config (declarative, hotplug-safe)..."

    mkdir -p "$PIPEWIRE_CONF_DIR"

    local primary="${PRIMARY_SINK:-}" secondary="${SECONDARY_SINK:-}"
    if [ -z "$primary" ] || [ "$primary" = "auto" ]; then
        primary=$(pactl list short sinks 2>/dev/null | awk '{print $2}' | grep "^alsa_output.usb-" | head -1)
    fi
    if [ -z "$secondary" ] || [ "$secondary" = "auto" ]; then
        secondary=$(pactl list short sinks 2>/dev/null | awk '{print $2}' | grep -E "^alsa_output.*hdmi" | head -1)
        if [ -z "$secondary" ]; then
            secondary=$(pactl list short sinks 2>/dev/null | awk '{print $2}' | grep "^alsa_output.pci-" | grep -v "^${primary}$" | head -1)
        fi
    fi

    if [ -z "$primary" ] || [ -z "$secondary" ]; then
        warning "Could not determine two sinks for the native combined sink config."
        warning "Copy examples/60-combined-sink.conf to $PIPEWIRE_CONF_DIR and edit stream.rules."
        return
    fi

    local p_re s_re
    p_re=$(sink_to_rule_regex "$primary")
    s_re=$(sink_to_rule_regex "$secondary")

    cat > "$PIPEWIRE_CONF_DIR/60-combined-sink.conf" <<EOF
# Native PipeWire combined sink - generated by install.sh on $(date +%F)
# The core creates this sink at every start; slaves attach/detach by rule,
# so device hotplug never requires an audio-stack restart.
# Primary:   $primary
# Secondary: $secondary
...
EOF
    success "Installed 60-combined-sink.conf (slaves: ${p_re} + ${s_re})"
    info "The combined sink is now created by PipeWire itself on every start."
}
...
configure_nvidia_integration() {
    ...
    if [ -x "$HOME/.screenlayout/apply-layout.sh" ]; then
        if grep -q "PIPEWIRE_ENSURE_DEFAULTS" "$HOME/.screenlayout/apply-layout.sh" 2>/dev/null; then
            success "NVIDIA display layout with non-disruptive audio integration already installed"
            return
        elif grep -q "RESET_PIPEWIRE_BIN" "$HOME/.screenlayout/apply-layout.sh" 2>/dev/null; then
            warning "apply-layout.sh has the OLD audio integration (full PipeWire restart on"
            warning "every display event - this disrupts audio and can crash PipeWire)."
            echo "  Edit ~/.screenlayout/apply-layout.sh: replace the reset-pipewire call"
            echo "  with a call to ~/.local/bin/pipewire-ensure-defaults, or re-run the"
            ...
            if [[ ! "$patch_layout" =~ ^[Nn] ]]; then
                ...
                if grep -q "nvidia-capture.sh" "$HOME/.screenlayout/apply-layout.sh" 2>/dev/null; then
                    ...
                    # v2: display events must NOT restart the audio stack. The
                    # combined sink is declarative; we only re-assert the HDMI
                    # card profile and re-apply defaults (non-disruptive).
                    if ! grep -q "PIPEWIRE_ENSURE_DEFAULTS" "$HOME/.screenlayout/apply-layout.sh" 2>/dev/null; then
...
PIPEWIRE_ENSURE_DEFAULTS="${PIPEWIRE_ENSURE_DEFAULTS:-$HOME/.local/bin/pipewire-ensure-defaults}"' "$HOME/.screenlayout/apply-layout.sh"

                        # Add HDMI handshake fix and defaults enforcement after the capture.sh call
    ...
    if [ -n "$PIPEWIRE_ENSURE_DEFAULTS" ] \&\& [ "$PIPEWIRE_ENSURE_DEFAULTS" != "none" ] \&\& [ -x "$PIPEWIRE_ENSURE_DEFAULTS" ]; then\
        "$PIPEWIRE_ENSURE_DEFAULTS" 2>\&1 | while IFS= read -r line; do\
            echo "$(date '"'"'+%Y-%m-%d %H:%M:%S'"'"') - [pipewire] $line"\
                        ...
                        success "Patched apply-layout.sh with non-disruptive audio integration"
                    fi
                else
