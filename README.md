...
## Architecture v2: Declarative Combined Sink

Earlier versions created the combined sink with `pactl load-module
module-combine-sink`. That module lives **inside pipewire-pulse**, so it died
on every service restart and broke (`Broken pipe`) whenever a slave device
disappeared — which is why everything (login, display hotplug, the watchdog)
had to keep re-running `reset-pipewire`, and why every display event caused a
full audio-stack restart that disconnected every application's streams
(frozen browser tabs) and could race PipeWire into a segfault.

v2 declares the combined sink in the **PipeWire core** via
`libpipewire-module-combine-stream` (`~/.config/pipewire/pipewire.conf.d/60-combined-sink.conf`,
generated by `install.sh`):

- The core creates `combined_out` at every start — it can never be "lost".
- Slaves are matched by **rule** (regex on `node.name`), so when the HDMI
  capture card re-handshakes or the USB mixer replugs, its stream detaches
  and reattaches automatically. **No restart, no script, no interruption**
  to apps playing through the combined sink.
- `reset-pipewire` becomes a manual recovery tool for genuine failures;
  routine events (login, hotplug) only run the new lightweight
  `pipewire-ensure-defaults`, which sets default sink/source and volumes
  without touching any service.
- The watchdog (v2) no longer depends on `pipewire.service` (a
  `Requires=` meant every restart also killed the watchdog mid-recovery),
  reacts to hard failures in seconds instead of 90+, and escalates:
  orderly restart → force kill + socket cleanup → desktop notification
  for a physical USB replug.
...
- ✅ Generate the **native combined sink config** (`60-combined-sink.conf`) from your devices
...
cp examples/pipewire-ensure-defaults.sh ~/.local/bin/pipewire-ensure-defaults
cp examples/reset-pipewire-nuclear.sh ~/.local/bin/reset-pipewire-nuclear
chmod +x ~/.local/bin/reset-pipewire ~/.local/bin/audio-status ~/.local/bin/reset-usb-audio ~/.local/bin/pipewire-watchdog ~/.local/bin/pipewire-ensure-defaults ~/.local/bin/reset-pipewire-nuclear

# Install the native combined sink config (EDIT the stream.rules first to
# match your devices — see the comments inside the file)
mkdir -p ~/.config/pipewire/pipewire.conf.d/
cp examples/60-combined-sink.conf ~/.config/pipewire/pipewire.conf.d/
...
If you use an HDMI capture card with the [NVIDIA X11 Multi-Monitor Toolkit](https://github.com/ChiefGyk3D/nvidia-display-layout), the combined audio sink keeps working across capture card hotplug automatically — both at startup and on hotplug.

### How It Works

The combined sink is created **declaratively by the PipeWire core** (`60-combined-sink.conf`), so when the capture card's HDMI sink disappears and reappears, its stream detaches and reattaches on its own — nothing needs to recreate the sink. The NVIDIA toolkit's `apply-layout.sh` script detects whether a capture card is connected and applies the correct display layout. When `pipewire-ensure-defaults` is installed to `~/.local/bin/`, the layout script will additionally:

1. Detect the capture card is connected
2. Apply the NVIDIA display layout (mirror to capture card)
3. Wait for the HDMI audio handshake and force the HDMI card profile active
4. Call `pipewire-ensure-defaults` to re-assert the default sink/source and volumes — **without restarting any audio service or touching running app streams**

When the capture card is **not** connected, the audio integration is skipped entirely.

> ⚠️ **Older installs** patched `apply-layout.sh` to call `reset-pipewire`, which fully restarted the audio stack on every display event — disconnecting all application audio and occasionally crashing PipeWire. Re-run this project's `install.sh` (or the NVIDIA toolkit's `setup-wizard.sh`) to upgrade to the non-disruptive integration.
...
2. **Install this project** (if not already):
   ...
   This installs `pipewire-ensure-defaults` to `~/.local/bin/` (which the NVIDIA toolkit automatically detects) and the declarative `60-combined-sink.conf`.
...
```bash
# Check the display monitor log for audio entries
cat /tmp/nvidia-display-monitor.log | grep pipewire

# Lines prefixed with [pipewire] indicate the defaults pass ran
```

### Disabling Integration

If you want to keep the scripts installed but prevent the audio pass from running on display changes, edit `~/.screenlayout/apply-layout.sh` and set:

```bash
PIPEWIRE_ENSURE_DEFAULTS="none"
```
...
**After nuclear reset**: With the native config installed, PipeWire recreates the combined sink itself as soon as it starts; run `pipewire-ensure-defaults` to re-assert defaults. The watchdog also verifies health on its next check.

### Want to remove the combined sink

**Native (v2)**: remove the config and restart PipeWire:
```bash
rm ~/.config/pipewire/pipewire.conf.d/60-combined-sink.conf
systemctl --user restart pipewire pipewire-pulse wireplumber
```

**Legacy (module-combine-sink)**:
```bash
# Find the module ID
...
- Probes the PipeWire core and pulse bridge every 15 seconds
- Hard failures (daemons unreachable) recover after one 5-second confirmation
- Soft failures (missing devices, unresponsive USB sink) recover after 3 strikes
- Escalates: orderly restart → force kill + socket cleanup → replug notification
...
### Custom combined sink parameters

**Native (v2, recommended)**: edit `~/.config/pipewire/pipewire.conf.d/60-combined-sink.conf`. The `stream.rules` regexes control which sinks join the combined output; `combine.props`/`stream.props` accept standard PipeWire node properties (channel map, latency, etc.). Restart PipeWire to apply. See the [module-combine-stream documentation](https://docs.pipewire.org/page_module_combine_stream.html).

**Legacy fallback**: edit the `pactl load-module` line in the `load_combined()` function:
...
The unit runs `pipewire-ensure-defaults` at login — it waits for the
core-created combined sink and enforces default sink/source and volumes
without restarting anything. (Earlier versions ran `reset-pipewire` here,
which restarted the freshly started audio stack at every login and could
race PipeWire into a crash.) If you installed the scripts elsewhere, update
the `ExecStart` path in the copied unit file accordingly.
...
The watchdog (v2):
- Probes the PipeWire core directly (`pw-cli`) **and** the pulse bridge (`pactl`) every 15 seconds — a hung core that refuses new connections is caught even when the process looks alive
- Hard failures (daemons unreachable) trigger recovery after a single 5-second confirmation; soft failures (missing devices, dead combined sink, unresponsive USB sink) after 3 strikes
- Recovery is tiered: orderly service restart → force kill + runtime socket cleanup → desktop notification asking for a physical USB replug
- Survives PipeWire restarts (no `Requires=` coupling) and treats external restarts as a grace period instead of a failure
- Automatically restarts if it crashes; logs all actions to the systemd journal
...
3. **Module Phase**:
   - If the native declarative config (`60-combined-sink.conf`) is
     installed, waits for the core-created `combined_out`, cleans up any
     leftover legacy module, and only applies defaults/volumes
   - Otherwise (legacy): unloads any previously saved combined module,
     loads `module-combine-sink` with specified slaves, and saves the new
     module ID for future unloading
